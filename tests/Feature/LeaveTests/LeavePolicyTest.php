...
it('shows the edit form for existing leave policy', function () {
    ...
    $response->assertOk()
        ->assertViewHas('leavePolicy', $this->leavePolicy);
});
...
it('returns 404 when trying to edit non-existent leave policy', function () {
    ...
    $response->assertRedirect(route('hr.leave-type.index'))
        ->assertSessionHasErrors([
            'leavePolicyID' => 'Politika e lejes me këtë ID nuk egziston.',
        ]);
});

it('updates leave policy with valid data', function () {
    $newData = [
        'leavePolicyID' => $this->leavePolicy->leavePolicyID,
        'annualQuota' => 25,
        ...
        'carryOverLimit' => 10,
        'restricedDays' => json_encode(['sunday']),
        'requirenments' => json_encode(['min_service' => 90]),
    ];

    $response = $this->post(route('hr.leave-policy.update'), $newData);

    $response->assertRedirect(route('hr.leave-type.index'))
        ->assertSessionHas('success', 'Politikat e pushimit u përditësuan me sukses.');
    $this->leavePolicy->refresh();

    expect($this->leavePolicy->annualQuota)->toBe(25)
        ->and($this->leavePolicy->maxConsecutiveDays)->toBe(15)
        ->and($this->leavePolicy->allowHalfDay)->toBe(0)
        ->and($this->leavePolicy->probationPeriodDays)->toBe(60)
        ->and($this->leavePolicy->carryOverLimit)->toBe('10.00')
        ->and($this->leavePolicy->restricedDays)->toBe(json_encode(['sunday']))
        ->and($this->leavePolicy->requirenments)->toBe(json_encode(['min_service' => 90]));
});

it('validates required fields when updating leave policy', function () {
    $response = $this->post(route('hr.leave-policy.update'), [
        'leavePolicyID' => $this->leavePolicy->leavePolicyID,
...
it('validates numeric fields to update policy data', function () {
    $invalidData = [
        'leavePolicyID' => $this->leavePolicy->leavePolicyID,
        'annualQuota' => 'not-a-number',
        'maxConsecutiveDays' => 'not-a-number',
    ...
    $response = $this->post(route('hr.leave-policy.update'), $invalidData);
...
it('validates minimum values to update policy numeric fields', function () {
    $invalidData = [
        'leavePolicyID' => $this->leavePolicy->leavePolicyID,
        'annualQuota' => -1,
        'maxConsecutiveDays' => 0, // min is 1 when present
...
it('returns error when trying to update non-existent leave policy', function () {
    $invalidId = 9999;
    $data = [
        'leavePolicyID' => $invalidId,
        'annualQuota' => 20,
    ...
    $response = $this->post(route('hr.leave-policy.update'), $data);

    $response->assertRedirect(route('hr.leave-type.index'));

    $response->assertSessionHasErrors(['leavePolicyID']);

    $errors = session('errors');
    expect($errors->first('leavePolicyID'))->toContain('Politika e lejes me këtë ID nuk egziston');
});
